class User:
	...
	def searchUserById(dbm, idUser):
		...
		userRes = User(dbm)
		if dbm is not None:
			res = dbm.runQuery("SELECT id, created_at, description, followers_count, friends_count, geo_enabled, location, name, protected, time_zone, url, verified FROM Users WHERE id = {}".format(idUser))
			try:
				if res is not None:
					row = res[0]
					userRes.set(idUser = row[0], created_at = row[1], description = row[2], followers_count = row[3], friends_count = row[4], geo_enabled = row[5], location = row[6], name = row[7], protected = row[8], time_zone = row[9], url = row[10], verified = row[11])
			except:
				pass
			return userRes
		else:
